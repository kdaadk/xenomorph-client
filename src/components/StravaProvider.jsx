...
const StreamTypes = ["velocity_smooth", "time", "distance", "latlng", "heartrate", "cadence", "altitude"];
...
class StravaProvider extends Component {
        ...
        return (
            <div>
                <ButtonGroup variant="contained" color="primary" aria-label="contained primary button group">
                    ...
                    <Button onClick={() => this._getStravaActivities()}>Get last activities from Strava</Button>
                </ButtonGroup>
            </div>
        );
    }
    ...
    _getStravaActivities = async () => {
        ...
        const listActivities = await stravaApi.athlete.listActivities({"per_page": "30", "after": timestampActivity}); // 01/01/2020: "1577836800"
        ...
        await api.insertActivities(newActivities)
            .then(async () =>  await api.insertStreams(newStreams))
            .then(() => window.location.reload())
    };
    
    _getStreamsAsync = async (id) => await stravaApi.streams.activity({"id": id, "types": StreamTypes});    
